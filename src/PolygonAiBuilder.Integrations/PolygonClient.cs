    ...
    TimeProvider timeProvider) : IPolygonClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var problems = Result<PolygonProblemWire[]>(document);
        return problems
    ...
    public Task<string> ViewResourceFileAsync(
        long problemId,
        string name,
        CancellationToken cancellationToken = default) =>
        SendTextProblemAsync("problem.viewFile", problemId,
            [Pair("type", "resource"), Pair("name", name)], cancellationToken);

    public async Task SaveResourceFileAsync(
        long problemId,
        string name,
        string content,
        CancellationToken cancellationToken = default)
    {
        using var document = await SendProblemAsync("problem.saveFile", problemId,
        [
            Pair("type", "resource"), Pair("name", name), Pair("file", content),
        ], cancellationToken);
    }
    ...
    private Task<string> SendTextProblemAsync(
        string methodName,
        long problemId,
        IEnumerable<KeyValuePair<string, string>> parameters,
        CancellationToken cancellationToken) =>
        SendTextAsync(methodName, parameters.Prepend(Pair("problemId", Invariant(problemId))), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requestBody = $"{signed.FormEncodedQuery}&apiSig={PolygonSignature.Encode(signed.ApiSignature)}";
        ...
        using (response)
        {
            ...
            {
                var safeComment = await ReadHttpErrorAsync(response, secrets, cancellationToken);
                throw new ExternalServiceException(
                    "Polygon",
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            return document.RootElement.TryGetProperty("comment", out var comment)
    ...
    private static T Result<T>(JsonDocument document)
    {
        if (!document.RootElement.TryGetProperty("result", out var result))
        {
            throw new ExternalServiceException(
                "Polygon",
                "invalid_response",
                "Polygon trả về phản hồi thiếu trường result cho API cần dữ liệu.");
        }
        ...
        try
        {
            return result.Deserialize<T>(SerializerOptions)
                ?? throw new ExternalServiceException(
                    "Polygon",
                    "invalid_response",
                    "Polygon trả về result không đúng cấu trúc.");
        }
        catch (JsonException exception)
        {
            throw new ExternalServiceException(
